...
class Ui_MainWindow(object):
	def setupUi(self, MainWindow):
		...
		self.verticalLayout = QtGui.QGridLayout(MainWindow)
		...
		self.horizontalLayout = QtGui.QVBoxLayout(self.scrollAreaWidgetContents)
		...
		self.fs = QtGui.QPushButton(self.frame)
		...
		self.dockWidgetContents = QtGui.QWidget(MainWindow)
		...
		self.list3.setFrameShape(QtGui.QFrame.StyledPanel)
		...
		self.list1 = QtGui.QListWidget(self.tab_2)
		...
		self.list2 = QtGui.QListWidget(self.tab_3)
		...
		self.progress = QtGui.QProgressBar(MainWindow)
		...
		#QtGui.QShortcut(QtGui.QKeySequence(QtCore.Qt.Key_Right), self.frame, self.hello_next)
		...
		QtGui.QShortcut(QtGui.QKeySequence("F"), self.frame, self.fullscreen)
		...
		#QtCore.QObject.connect(self.line3, QtCore.SIGNAL(_fromUtf8("returnPressed()")), self.setchapter)
		#QtCore.QObject.connect(self.line2, QtCore.SIGNAL(_fromUtf8("returnPressed()")), self.setpage)
		...
		#self.scrollArea.verticalScrollBar().setPageStep(30)
	...
	def download_thread_finished(self,indx,p,n):
		...
		#	if self.downloadWget[0].isFinished():
				#self.downloadWget[0].terminate()
		#		del self.downloadWget[0]
		#		if self.downloadWget:
		#			self.downloadWget[0].start()
		#if n == 0:
		#	self.createLabel(p,n)
...
if __name__ == "__main__":
	...
	if os.path.exists('/usr/share/ReadManga'):
		sys.path.append('/usr/share/ReadManga')
	try:
		m = os.listdir('/tmp/ReadManga/')
		for i in m:
	...
	if os.path.isfile(home+"/"+site+"/"+name+".txt"):
		...
		if ui.downloadWgetNew:
			for i in ui.downloadWgetNew:
				if not i.isFinished():
					i.terminate()
			ui.downloadWgetNew[:]=[]
		
		if ui.downloadWgetUrlNew:
			for i in ui.downloadWgetUrlNew:
				if not i.isFinished():
					i.terminate()
